...
<body x-data="{ showModal1: false , showModal2: false, showModal3: false  }" :class="{'overflow-y-hidden': showModal1}">
        ...
        <div class="relative sm:w-3/4 md:w-1/2 lg:w-1/3 mx-2 sm:mx-auto my-10 opacity-100">
            <div class="relative bg-white shadow-lg rounded-lg text-gray-900 z-20" @click.away="showModal2 = false"
                ...
                <div class="p-3  mt-2 text-center space-x-4 md:block">
                    ...
                    <a href="" id="deleteStudent"><button
    ...
    <div class="fixed inset-0 w-full h-full z-20 bg-black bg-opacity-50 duration-300 overflow-y-auto"
        ...
        <div class="relative sm:w-3/4 md:w-1/2 lg:w-1/3 mx-2 sm:mx-auto my-10 opacity-100">
            <div class="relative bg-white shadow-lg rounded-md text-gray-900 z-20" @click.away="showModal3 = false"
                ...
                x-transition:leave-end="scale-0">
                <header class="flex items-center justify-between p-2">
                    <h2 class="font-semibold">THÊM SINH VIÊN </h2>
                ...
                <form action="../student/addStudent" method="POST">
                    <div class="px-5 py-7">
                        <label class="font-semibold text-sm text-gray-600 pb-1 block">Tên sinh viên</label>
                        <input type="text"
                        ...
                        <label class="font-semibold text-sm text-gray-600 pb-1 block">Lớp</label>
                        
                        <select name="class" class=" border rounded-lg px-3 py-3 mt-1 mb-5 text-sm w-full"  >
                            <?php
                            if(isset($data['student'])){
                                $property_types = array();
                                foreach($data['student'] as $Data1){
                                    if ( in_array($Data1['class_name'], $property_types) ) {
                                        continue;
                                    }
                                    $property_types[] = $Data1['class_name'];
                            ?>
                            <option value="<?php echo $Data1['class_id']?>"><?php echo $Data1['class_name']?></option>
                            <?php
                                }}
                            ?>
                        </select>  
                        ...
                        <table class="min-w-max w-full table-auto">
                            <thead>
                                <tr class="bg-gray-200 text-gray-600 uppercase text-sm leading-normal">
                                    ...
                                    <th class="py-3 px-6 text-center">Lớp</th>
                                        ...
                                        <div class="flex item-center justify-center">
                                            ...
                                            <div class="w-4 mr-2 transform hover:text-purple-500 hover:scale-110">
                                                ...
                                                <a herf="javascript:void(0)"
                                                    onclick="edit_Data(<?php echo $data["students_id"] ?>,'<?php echo $data["students_name"] ?>','<?php echo $data["gender"] ?>','<?php echo $data["address"] ?>')">
                                            ...
                                            <div class="w-4 mr-2 transform hover:text-purple-500 hover:scale-110">
                                                <a herf="javascript:void(0)"
                                                    onclick="delete_Data(<?php echo $data["students_id"] ?>)">
    ...
    <script>
    function edit_Data(student_id, student_name, gender, address) {
    ...
    function delete_Data(student_id) {
        $student_id = "../student/deleteStudent/" + student_id;
        document.getElementById("deleteStudent").setAttribute("href", $student_id);
    }
