...
<body x-data="{ showModal1: false , showModal2: false, showModal3: false  }" :class="{'overflow-y-hidden': showModal1}">
    ...
    <div class="fixed inset-0 w-full h-full z-20 bg-black bg-opacity-50 duration-300 overflow-y-auto"
        ...
        <div class="relative sm:w-3/4 md:w-1/2 lg:w-1/3 mx-2 sm:mx-auto my-10 opacity-100">
            <div class="relative bg-white shadow-lg rounded-md text-gray-900 z-20" @click.away="showModal1 = false"
                ...
                <form action="../subject/editSubject" method="post">
                    <div class="px-5 py-7">
                        <input hidden class="border rounded-lg px-3 py-2 mt-1 mb-5 text-sm w-full" type="text"
                            id="subject_id" name="subject_id" />
                        <label class="font-semibold text-sm text-gray-600 pb-1 block">Môn học</label>
                        <input type="text" class="border roundedx-lg px-3 py-2 mt-1 mb-5 text-sm w-full"
                            id="subject_name" name="subject_name" />
                        <label class="font-semibold text-sm text-gray-600 pb-1 block">Số tín chỉ</label>
                        <input type="text" class="border rounded-lg px-3 py-2 mt-1 mb-5 text-sm w-full" id="credit"
                            name="credit" />
    ...
    <div class="fixed inset-0 w-full h-full z-20 bg-black bg-opacity-50 duration-300 overflow-y-auto"
        ...
        <div class="relative sm:w-3/4 md:w-1/2 lg:w-1/3 mx-2 sm:mx-auto my-10 opacity-100">
            <div class="relative bg-white shadow-lg rounded-md text-gray-900 z-20" @click.away="showModal3 = false"
                ...
                x-transition:leave-end="scale-0">
                <header class="flex items-center justify-between p-2">
                    <h2 class="font-semibold">THÊM MÔN HỌC</h2>
                ...
                <form id="editDetails">
                    <div class="px-5 py-7">
                        <label class="font-semibold text-sm text-gray-600 pb-1 block">Môn học</label>
                        <input type="text"
                            class="edit_subject_details border rounded-lg px-3 py-2 mt-1 mb-5 text-sm w-full"
                            name="subject_name" />
                        <label class="font-semibold text-sm text-gray-600 pb-1 block">Số tín chỉ</label>
                        <input type="text" name="credit"
                            class="edit_credit_details border rounded-lg px-3 py-2 mt-1 mb-5 text-sm w-full" id="credit"
                            name="credit" />
                        <button name="btnSubmit" type="submit"
                            class="transition duration-200 bg-blue-500 hover:bg-blue-600 focus:bg-blue-700 focus:shadow-sm focus:ring-4 focus:ring-blue-500 focus:ring-opacity-50 text-white w-full py-2.5 rounded-lg text-sm shadow-sm hover:shadow-md font-semibold text-center inline-block">
                            <span class="inline-block mr-2">Cập nhập</span>
                ...
                <div class="flex flex-col sm:flex-row mt-10">
                    <div class="sm:w-1/3 text-center sm:pr-8 sm:py-8">
                        ...
                        <div class="flex flex-col items-center text-center justify-center">
                            <h2 class="font-medium title-font mt-4 text-gray-900 text-lg">QUẢN LÝ MÔN HỌC</h2>
                        ...
                        <table class="min-w-max w-full table-auto">
                            <thead>
                                <tr class="bg-gray-200 text-gray-600 uppercase text-sm leading-normal">
                                    ...
                                    <th class="py-3 px-6 text-left">Số tín chỉ</th>
                                        ...
                                        <div class="flex item-center justify-center">
                                            ...
                                            <div class="w-4 mr-2 transform hover:text-purple-500 hover:scale-110">
                                                ...
                                                <a herf="javascript:void(0)"
                                                    onclick="edit_Data(<?php echo $data["subject_id"] ?>,'<?php echo $data["subject_name"] ?>',<?php echo $data["credit"] ?>)">
                                            ...
                                            <div class="w-4 mr-2 transform hover:text-purple-500 hover:scale-110">
                                                <a herf="javascript:void(0)"
                                                    onclick="delete_Data(<?php echo $data["subject_id"] ?>)">
    ...
    function edit_Data(subject_id, subject_name, credit) {
        $('#subject_id').val(subject_id);
        $('#subject_name').val(subject_name);
        $('#credit').val(credit);
    }
    function delete_Data(subject_id) {
        var subject_id = "../subject/deleteSubject/" + subject_id;
        document.getElementById("deleteSubject").setAttribute("href", subject_id);
    }
    $('#editDetails').submit(function(e) {
        e.preventDefault();
        var pattern_subjectName=/^([a-zA-Z0-9ÀÁÂÃÈÉÊÌÍÒÓÔÕÙÚĂĐĨŨƠàáâãèéêìíòóôõùúăđĩũơƯĂẠẢẤẦẨẪẬẮẰẲẴẶẸẺẼỀỀỂưăạảấầẩẫậắằẳẵặẹẻẽềềểỄỆỈỊỌỎỐỒỔỖỘỚỜỞỠỢỤỦỨỪễệỉịọỏốồổỗộớờởỡợụủứừỬỮỰỲỴÝỶỸửữựỳỵỷỹ\s]{3,100})$/i;
        var pattern_credit=/^[1-9]{1,2}$/
        var subject_name = $('.edit_subject_details').val();
        var credit = $('.edit_credit_details').val();
        if (subject_name == '' || credit == '') {
            ...
            Swal.fire({
                ...
                title: 'Lỗi...',
                timer: 1500,
                text: 'Vui lòng nhập chữ số vào ô tín chỉ',
                showConfirmButton: false,
            });
        }else if(!pattern_subjectName.test(subject_name)){
            Swal.fire({
                icon: 'error',
                title: 'Lỗi...',
                timer: 1500,
                text: 'Tên môn học không được có kí tự đặc biệt và độ dài từ 3 đến 100 kí tự',
                showConfirmButton: false,
            });
        }else if(!pattern_credit.test(credit)){
            Swal.fire({
                icon: 'error',
                title: 'Lỗi...',
                timer: 1500,
                text: 'Số tín chỉ chỉ nhập số và từ 1 đến 2 kí tự',
                showConfirmButton: false,
            });
        }else{
            $.ajax({
